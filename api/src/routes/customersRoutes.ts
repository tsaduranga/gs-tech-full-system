...
customersRouter.get("/", requirePermission("customers.read"), async (req, res, next) => {
  try {
    const qp = listQuerySchema.parse(req.query);
    const offset = (qp.page - 1) * qp.pageSize;
    const { rows, total } = await mastersModel.customers.listPaginated({
      search: qp.q,
      limit: qp.pageSize,
      offset,
    });
...
customersRouter.get("/:id", requirePermission("customers.read"), async (req, res, next) => {
  try {
    ...
    const row = await mastersModel.customers.get(id);
...
customersRouter.patch("/:id", requirePermission("customers.write"), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id) || id < 1) throw new HttpError(400, "Invalid customer id");
    ...
    await mastersModel.customers.update(id, {
...
customersRouter.delete("/:id", requirePermission("customers.write"), async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    if (Number.isNaN(id) || id < 1) throw new HttpError(400, "Invalid customer id");
    const exists = await mastersModel.customers.get(id);
