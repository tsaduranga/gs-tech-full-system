...
export default function SuppliersPage() {
  ...
  const loadList = useCallback(async () => {
    ...
    const res = await apiJson<SupplierListResponse>(`/suppliers?${qs.toString()}`);
    setLoading(false);
    if (!res.ok || !res.data?.items) {
      setListError(res.error ?? "Failed to load suppliers");
      setList([]);
      setTotal(0);
      return;
    }
  ...
  const onSubmitValid: SubmitHandler<SupplierFormValues> = async (data) => {
    ...
    try {
      if (editingId == null) {
        const res = await apiJson<{ id: number }>("/suppliers", {
          method: "POST",
        ...
        if (!res.ok) {
          form.setError("root", {
            type: "server",
            message: res.error ?? "Could not create supplier",
          });
          setSubmitting(false);
          return;
        }
      ...
      const res = await apiJson(`/suppliers/${editingId}`, {
        method: "PATCH",
        body: JSON.stringify(payload),
      });
      ...
      if (!res.ok) {
        form.setError("root", {
          type: "server",
          message: res.error ?? "Could not update supplier",
        });
        return;
      }
      setDialogOpen(false);
  ...
  const onSubmitInvalid: SubmitErrorHandler<SupplierFormValues> = () => {};

  async function deleteSupplier(row: SupplierRow) {
    const ok = await confirm({
      title: "Delete supplier",
      description: `Remove supplier "${row.name}" from lists? Historical records are kept.`,
      confirmLabel: "Delete",
      destructive: true,
    });
    if (!ok) return;
    const res = await apiJson(`/suppliers/${row.id}`, { method: "DELETE" });
    if (!res.ok) {
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <div className="flex max-w-md flex-1 flex-col gap-2">
            <Label htmlFor="sup-filter">Filter</Label>
          ...
          <div className="flex min-w-[140px] flex-col gap-2">
            <Label htmlFor="sup-page-size">Rows per page</Label>
            <SearchableNumPicker
              id="sup-page-size"
          ...
          >
            ...
            <div className="flex-1 space-y-4 overflow-y-auto px-4 pb-1">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="sup-active" className="cursor-pointer font-normal">
                  Active (appears when creating purchase orders)
                </Label>
              </div>
